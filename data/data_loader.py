...
class CIFAR100C(Dataset):
    def __init__(self, data_root='/data/temp_zenglin/data/CIFAR-100-C/', transform=None, classes=[]):
        ...
        self.data = []
        self.label = []
        # for each fileThe first 10,000 images in each .npy are the test set images corrupted at severity 1, and the last 10,000 images are the test set images corrupted at severity five.labels.npy is the label file for all other image files.
        label = np.load('{}/labels.npy'.format(data_root))
        files = glob(os.path.join(data_root, '*.npy'))
        for file in files:
            if 'labels' in file: continue # skip label files
            file = np.load(file)
        ...
        for i in range(len(self.label)):
            self.label[i] = cls_list.index(self.label[i])
        # filter data and label based on classes
        if len(classes) > 0:
            mask = np.isin(self.label, classes)
...
if __name__ == '__main__':
    test_set = CIFAR100C(classes=[1, 2, 11, 33])
    print(len(test_set))
