...
class StyleTransfer:
    ...
    def __call__(self, image, style=None, alpha=0.5):
        n, c, h, w = image.shape
...
def style_transfer2(image, style=None, alpha=0.5):
    n, c, h, w = image.shape
    content = image.detach()
    if style is not None:
        style = style.detach()
    ...
    with torch.no_grad():
        stylized_image = []
        for i in range(n):
            stylized_image.append(PT.pdf_transfer(img_arr_in=content[i],
                                  img_arr_ref=style[i],
                                  regrain=True))
    return torch.tensor(stylized_image, dtype=image.dtype, device=image.device)